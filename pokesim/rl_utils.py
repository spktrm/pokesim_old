...
class EntropySchedule:
    ...
    def __call__(self, learner_step: int) -> Tuple[float, bool]:
        ...
        update_target_net = jnp.logical_and(
            learner_step > 0,
            jnp.sum(learner_step + 1 == iteration_start + iteration_size),
        )
        alpha = jnp.minimum(
...
class SGDTowardsModel:
    def __init__(self, from_model: nn.Module, to_model: nn.Module, lr: float):
    ...
    @torch.no_grad()
    def step(self):
        for param_from, param_to in zip(
            self._from_model.parameters(), self._to_model.parameters()
        ):
            grad = param_from - param_to
            param_from.sub_(grad, alpha=self._lr)
